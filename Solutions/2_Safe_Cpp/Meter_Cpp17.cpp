...
// 2. The constructor(s) should control conversions (e.g. via std::enable_if)
...
// 6. The template parameter 'T' should be checked by means of a 'static_assert'
...
class Meter final
{
 public:
   ...
   template< typename U
           , typename = std::enable_if_t< std::is_same_v<T,U> > >  // Constraint on the parameter: no conversion allowed!
...
int main()
{
   ...
   static_assert( std::is_same_v<decltype(meter1),Meter<int> const> );
   static_assert( meter1.get() == 100 );

   constexpr Meter meter2{ 4.0 };  // Meter<double>
   static_assert( std::is_same_v<decltype(meter2),Meter<double> const> );
   ...
   static_assert( std::is_same_v<decltype(meter5),Meter<int> const> );
