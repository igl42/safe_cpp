...
{
   template< typename T >
   constexpr T& operator+=( this T& lhs, T const& rhs )
   ...
   template< typename T >
   constexpr T operator+( this T const& lhs, T const& rhs )
      noexcept( noexcept( lhs.get() + rhs.get() ) )
   {
...
{
   template< typename T >
   void print( this T const& self, std::ostream& os )
   {
      os << self.get();
   }
};
...
{
   template< typename T >
   constexpr bool operator==( this T const& self, T const& rhs )
   {
   ...
   : public Skills...
{
 ...
 private:
   ...
   friend std::ostream& operator<<( std::ostream& os, StrongType const& s )
   {
      s.print( os );
      return os;
   }
